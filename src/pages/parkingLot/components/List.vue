    ...
    <el-menu 
      ...
      <div v-for="(item,i) in boundayList" :key="i">
        <el-submenu 
          ...
          <template slot="title">
            <p>
              <span class="unload-area" :title="item.areaName"><i class="iconfont icon-tingchechangzuoye park_left_icon"></i>{{item.areaName}}</span>
            </p>
          </template>
          <div v-for="(berthArea,j) in item.parkGrps" :key="i+'-'+j">
            <el-submenu 
              class="other item_menu2"
             :class="{'active-item':active.length==2&&active[0]==i&&active[1]==j}"
              :index="i+'-'+j">
              <template slot="title">
                  <p :title="berthArea.grpSn +berthPlaceTitle(berthArea)" >
                    <span class="dock-area">
                      <i class="iconfont icon-tingchechang park_left_icon park_area_icon" :class="'active'+berthArea.workStatusValue"></i>
                      G#{{berthArea.grpSn}}
                    </span>
                  </p>
              </template>
              <div v-for="(berthPlace,k) in berthArea.parkSpaces" :key="i+'-'+j +'-'+k">
                <el-menu-item 
                  ...
                  <p :title="berthPlace.name+berthPlaceTitle(berthPlace)">
                    <i class="iconfont icon-dianwei" :class="'open_status'+berthPlace.workStatusValue + (berthPlace.used?' active':'')"></i>
                    <span class="dock-place">
                      <span class="dock-place-name">{{berthPlace.name}}</span>
                      <!-- 允许停靠/禁止停靠 -->
                      <i v-if="berthPlace.allowParking" class="element_status stop_status1">允许停靠</i>
                      <i v-else class="element_status stop_status0">禁止停靠</i>
                    </span>
                  </p>
                </el-menu-item>
              </div>
  ...
  methods: {
    ...
    async getList(refresh) {
      try {
        ...
        var list = data.filter(e=>e.subShowTypeSn==601);

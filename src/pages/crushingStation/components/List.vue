    ...
    <el-menu 
      ...
      <div v-for="(item,i) in boundayList" :key="i">
        <el-submenu 
          ...
          <template slot="title">
            <p>
              <span class="unload-area" :title="item.areaName"><i class="iconfont icon-posuizhanzuoye park_left_icon"></i>{{''+item.areaName}}</span>
            </p>
          </template>
                  ...
                  <p>
                    <span 
                      ...
                      v-if="berthArea.bindDevice">
                        <i 
                          class="iconfont icon-posuiji park_left_icon park_area_icon" 
  ...
  methods: {
    ...
    async getList(refresh) {
      try {
        ...
        var data = result||[];
        var list = data.filter(e=>e.subShowTypeSn==603);

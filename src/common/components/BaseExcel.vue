...
export default {
  ...
  methods: {
    ...
    jsonToXLS(data) {
      ...
      if (this.title != null) {
        xlsData += this.parseExtraData(
          this.title,
          '<tr><th colspan="' + colspan + '" style="font-size:20px;height:40px;">${data}</th></tr>'
        );
      }
    ...
    /*
		htmlToXLS
		---------------
		Transform html data into an xml document with MS Excel format, sadly
		it shows a prompt when it opens, that is a default behavior for
		Microsoft office and cannot be avoided. It's recommended to use CSV format instead.
		*/
    htmlToXLS(data) {
